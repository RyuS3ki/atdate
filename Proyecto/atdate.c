...
int udp_client(char *host, int port, int debug){
  ...
  n = sendto(clientfd, NULL, 0, 0, (const struct sockaddr *) &serveraddr, sizeof(const struct sockaddr *));
  if (n < 0) {
  ...
  /* read answer from the server */
  n = recvfrom(clientfd, &buf, sizeof(uint32_t), 0, (const struct sockaddr *) &serveraddr, sizeof(const struct sockaddr *));
  if (n < 0) {
    perror("ERROR reading from socket");
    exit(0);
  }
