...
/* Usage function */
void usage(){
	fprintf(stderr, "usage: atdate [-h serverhost] [-p port] [-m cu|ct|s|su] [-d] [-x value]\n");
	exit(1);
}
...
int tcp_server(int debug){
  ...
  /* wait for a connection request */
    ...
    //printf("server got connection from %s (%s)\n", hostp->h_name, hostaddrp);
		...
		// Reading datagram from client

		char received[10];
		int brcvd;
		uint32_t x;
		brcvd = recv(new_fd, received, sizeof(received), 0);
    if( brcvd < 0 ){
      ...
      if(debug) printf("Wrong type of datagram received, discarding...\n");
	      ...
	      time_t own_time;
				x = *received;
				printf("The value of x received is: %ld\n", x);
		      ...
		      }while( (x != 0) && (n > 0) ); // If n<0 -> Client closed connection
	      	...
	      	if(n<0){
	        	...
	        	printf("Date & time correctly sent\n");
					...
					own_time = time(NULL); // Getting the server's time
					own_time = own_time;
					if(debug) printf("Adjusting time to NTP timebase and calculating diff\n");
					time_send = htonl(own_time + LINUX_TIMEBASE - x); // Adjusted
					/* Sending time */
					if(debug) printf("Sending...\n");
					n = send(new_fd, &time_send, sizeof(uint32_t), 0);
					if(n<0){
...
/* TCP Client function */
int tcp_client(char *host, int port, int debug, uint32_t x){
  ...
  if (connect(clientfd, (struct sockaddr *) &serveraddr, sizeof(serveraddr)) < 0) {
	...
	uint32_t x_hton = htonl(x);

	/* Send value of x to server */
  n = send(clientfd, (void *) &x_hton, sizeof(uint32_t), 0);
  if (n < 0) {
  ...
  if(debug) printf("Waiting for server's answer\n");
	...
	int brcvd;
	do{
		brcvd = recv(clientfd, &buf, BUFSIZE, 0);
    if (brcvd < 0) {
      perror("ERROR receiving");
      exit(0);
    }else{
...
int main(int argc, char* const argv[]) {
   ...
   */
  while((opt = getopt(argc, argv, "dh:p:m:x:")) != -1){
    switch(opt){
      case 'h':
        host = optarg;
        break;
